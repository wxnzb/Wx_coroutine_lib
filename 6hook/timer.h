...
// };
// }
#ifndef _SYLAR_TIMER_H
#define _SYLAR_TIMER_H
#include <cstdint>
#include <functional>
#include <vector>
#include <memory>
...
namespace sylar
{
    ...
    public:
        ...
        std::shared_ptr<Timer> addConditionTimer(uint64_t ms, std::function<void()> cb,  std::weak_ptr<void> weakcond,bool curring=false);
        // 堆中是否还有timer
        bool hasTimer();
        // 取出所有时间的回调函数
        void listExpiredCb(std::vector<std::function<void()>> &cbs);
